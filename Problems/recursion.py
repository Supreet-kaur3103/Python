...
print("////////////////////")
#to understand the abobe program better
def is_even(x):
  if x == 0:
    print(f"is_even({x}): True")
    return True
  else:
    print(f"is_even({x}): is_odd({x-1})")
    return is_odd(x-1)

def is_odd(x):
  print(f"is_odd({x}): not is_even({x})")
  return not is_even(x)
...
def convert(num): 
    # Base case: If num is 0, return 0 (binary representation of 0)
    if num == 0:
        return 0
    
    # Recursive case
    # Calculate the remainder of num divided by 2 (rightmost binary digit)
    # Multiply it by 10 and add the recursive call to convert(num // 2)
    return (num % 2 + 10 * convert(num // 2)) 

# Prompt the user to enter a decimal number
a = int(input())

# Call the convert function with the input number and print the result
print(convert(a))
